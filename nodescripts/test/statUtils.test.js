...
describe('Update Plugin Stats', ()=>{
    beforeEach(()=>{
        ...
        const stub_readfile_TEXT = sinon.stub(readFile, "TEXT");
        const stub_readfile_JSON = sinon.stub(readFile, "JSON");
        const stub_writeFile = sinon.stub(writeFile, "writeJSONSync");
        const stub_axios = sinon.stub(axios, "get");

        stub_writeFile.resolves(true)        

        stub_readfile_TEXT.resolves(plugin_md);
        stub_readfile_JSON.resolves(prev_plugin_stat_json);
