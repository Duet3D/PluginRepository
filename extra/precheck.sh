...
# 2. Check if README.md or PLUGIN.md is present on the specified repo
    ...
    res=$(if [[ "$status_code" = 200 ]]; then echo "OK"; else echo "NOT OK"; fi)
...
# 3. Make sure there is at least one release with at least one ZIP file
    ...
    res=$(if [[ "$ext" = zip ]]; then echo "OK"; else echo "NOT OK"; fi)
...
# 5. Make sure plugin.json exists
    res=$((ls plugin.json >> /dev/null 2>&1  && echo "OK") || echo "NOT OK")
...
# 6. Ensure properties id, name, author are present. Validate them as specified in https://github.com/Duet3D/DuetWebControl/blob/master/src/plugins/manifest.js
    ...
    res=$(if [[ "$plugin_id" != null ]]&&[[ "$plugin_id_n" < 32 ]]; then echo "OK"; else echo "NOT OK"; fi)
    ...
    res=$(if [[ "$plugin_name" != null ]]&&[[ "$plugin_name_n" < 64 ]]; then echo "OK"; else echo "NOT OK"; fi)
    ...
    res=$(if [[ "$plugin_author" != null ]]&&[[ "$plugin_author_n" > 0 ]]; then echo "OK"; else echo "NOT OK"; fi)
